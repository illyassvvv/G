...
abstract class D {
  ...
  static const _base = TextStyle(decoration: TextDecoration.none, decorationColor: Colors.transparent);
...
class Match {
  ...
  const Match({required this.id, required this.league, required this.home, required this.homeEn,
    required this.homeLogo, required this.away, required this.awayEn, required this.awayLogo,
    required this.score, required this.time, required this.date, required this.isLive, required this.hasChannels});
...
class _RootShellState extends State<RootShell> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _tabAnim = AnimationController(vsync: this, duration: const Duration(milliseconds: 700))..forward();
    _init();
  }
  @override
  ...
  void _openPlayer(Channel ch) {
    ...
    Navigator.of(context).push(_premiumRoute(PlayerScreen(
      channel: ch, isFavorite: _favIds.contains(ch.id),
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Stack(children: [
      AnimatedSwitcher(
        ...
        switchInCurve:  D.snap,
        switchOutCurve: D.smooth,
        transitionBuilder: (child, anim) {
          final isIncoming = (child.key as ValueKey<int>).value == _tab;
          if (isIncoming) {
            return FadeTransition(
              opacity: CurvedAnimation(parent: anim, curve: Curves.easeOut),
              child: ScaleTransition(
                scale: Tween(begin: 0.965, end: 1.0).animate(CurvedAnimation(parent: anim, curve: D.snap)),
                child: child,
              ),
            );
          }
        ...
        child: KeyedSubtree(key: ValueKey(_tab), child: [
          HomeScreen(groups: _groups, matches: _matches, groupsLoading: _groupsLoading, matchLoading: _matchLoading, onOpenPlayer: _openPlayer, onGoMatches: () => _setTab(1), onRefresh: _refresh),
          MatchesScreen(matches: _matches, loading: _matchLoading, showScores: _settings['showScores'] as bool? ?? true, onRefresh: _loadMatches),
          FavoritesScreen(channels: _favChs, onOpenPlayer: _openPlayer, onToggleFav: _toggleFav),
          SettingsScreen(settings: _settings, onChanged: (s) async { setState(() => _settings = s); await Prefs.saveSettings(s); }),
        ][_tab]),
      ),
      Positioned(
        ...
        child: RepaintBoundary(
          child: SlideTransition(
            position: Tween(begin: const Offset(0, 1), end: Offset.zero).animate(CurvedAnimation(parent: _tabAnim, curve: D.snap)),
        ...
        child: BackdropFilter(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: const Color(0xFF161618).withValues(alpha: 0.86),
              borderRadius: BorderRadius.circular(D.r36),
              border: Border.all(color: Colors.white.withValues(alpha: 0.09), width: 0.5),
              boxShadow: [
                BoxShadow(color: Colors.black.withValues(alpha: 0.70), blurRadius: 64, spreadRadius: -8, offset: const Offset(0, 24)),
                BoxShadow(color: D.blue.withValues(alpha: 0.04), blurRadius: 40),
              ],
            ),
                  ...
                  TweenAnimationBuilder<double>(
                    ...
                    builder: (_, pos, __) => Positioned(
                      ...
                      child: DecoratedBox(decoration: BoxDecoration(
                        color: D.blue.withValues(alpha: 0.16),
                        borderRadius: BorderRadius.circular(28),
                        border: Border.all(color: D.blue.withValues(alpha: 0.30), width: 0.5),
                        boxShadow: [BoxShadow(color: D.blue.withValues(alpha: 0.22), blurRadius: 14)],
                      )),
                    ),
                  ),
                  Row(children: [
...
class _PillItem extends StatelessWidget {
  ...
  final String label;
  final ValueChanged<int> onTap;
  const _PillItem({required this.index, required this.selected, required this.icon, required this.activeIcon, required this.label, required this.onTap});
      ...
      child: _Tap(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          AnimatedDefaultTextStyle(
            ...
            style: TextStyle(fontSize: 10, fontWeight: active ? FontWeight.w600 : FontWeight.w400, color: active ? D.blue : D.lbl3, letterSpacing: -0.2, decoration: TextDecoration.none),
            child: Text(label),
          ),
        ]),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(
      ...
      slivers: [
        ...
        SliverToBoxAdapter(child: _FadeIn(delay: 0, child: Padding(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: D.blue.withValues(alpha: 0.40), blurRadius: 18, offset: const Offset(0, 5))],
              ),
              child: const Icon(CupertinoIcons.person_fill, size: 20, color: Colors.white),
            ),
          ]),
        ))),
        SliverToBoxAdapter(child: _FadeIn(delay: 60, child: Padding(
        ...
        if (groupsLoading)
          ...
          ...groups.asMap().entries.map((e) => SliverToBoxAdapter(
            child: _FadeIn(delay: 80 + e.key * 50, child: _ChannelGroupRow(group: e.value, onOpen: onOpenPlayer)),
          )),
...
class _FeaturedCardState extends State<_FeaturedCard> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    if (widget.loading) {
      return Container(height: 208, decoration: BoxDecoration(color: D.s2, borderRadius: BorderRadius.circular(D.r24)));
    }
        ...
        child: AnimatedBuilder(
          ...
          builder: (_, child) => Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(color: D.blue.withValues(alpha: 0.07 + 0.10 * _glow.value), blurRadius: 44, offset: const Offset(0, 16)),
          ...
          child: ClipRRect(
            ...
            child: Stack(children: [
              ...
              Positioned(top: -70, right: -70, child: Container(width: 200, height: 200, decoration: BoxDecoration(shape: BoxShape.circle, gradient: RadialGradient(colors: [Colors.white.withValues(alpha: 0.05), Colors.transparent])))),
              Positioned(bottom: -50, left: -50, child: Container(width: 160, height: 160, decoration: BoxDecoration(shape: BoxShape.circle, gradient: RadialGradient(colors: [D.blue.withValues(alpha: 0.07), Colors.transparent])))),
              Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(D.r24),
        ...
        Expanded(
          child: Container(
            decoration: BoxDecoration(
              ...
              border: Border.all(color: Colors.white.withValues(alpha: 0.09), width: 0.5),
              boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.44), blurRadius: 16, offset: const Offset(0, 8))],
            ),
            child: Stack(children: [
              Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(D.r20),
                gradient: LinearGradient(
                  ...
                  colors: [Colors.white.withValues(alpha: 0.07), Colors.transparent, Colors.black.withValues(alpha: 0.10)],
                ),
              ))),
              Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(D.r20),
                gradient: LinearGradient(begin: Alignment.topCenter, end: const Alignment(0, 0.35), colors: [Colors.white.withValues(alpha: 0.04), Colors.transparent]),
              ))),
              Padding(padding: const EdgeInsets.all(D.g16), child: Center(child: _NetImg(url: channel.logo, w: 62, h: 62))),
...
class MatchesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final m in matches) { final k = _d(m.date) == today ? 'Today' : _d(m.date) == tomorrow ? 'Tomorrow' : _d(m.date); grouped.putIfAbsent(k, () => []).add(m); }
    return CustomScrollView(
      physics: const BouncingScrollPhysics(parent: AlwaysScrollableScrollPhysics()),
      slivers: [
        CupertinoSliverRefreshControl(onRefresh: onRefresh),
        SliverToBoxAdapter(child: SizedBox(height: top + D.g8)),
        SliverToBoxAdapter(child: _FadeIn(delay: 0, child: Padding(padding: const EdgeInsets.fromLTRB(D.g20, 0, D.g20, D.g24), child: Text('Matches', style: D.hero)))),
        if (loading)
          const SliverToBoxAdapter(child: _Skeleton())
          ...
          ...grouped.entries.toList().asMap().entries.map((me) => SliverToBoxAdapter(
            child: _FadeIn(delay: 60 + me.key * 60, child: _MatchSection(label: me.value.key, matches: me.value.value, showScores: showScores)),
          )),
        const SliverToBoxAdapter(child: SizedBox(height: 120)),
      ],
    );
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(
      ...
      slivers: [
        ...
        if (channels.isEmpty)
          SliverFillRemaining(child: _FadeIn(delay: 60, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            _glassCircle(const Icon(CupertinoIcons.heart, size: 36, color: D.lbl3)),
          ...
          SliverPadding(
            ...
            sliver: SliverList(delegate: SliverChildBuilderDelegate(
              (_, i) {
                final ch = channels[i];
                return _FadeIn(delay: 40 + i * 40, child: _FavRow(
...
Widget _glassCircle(Widget child) => ClipRRect(
  borderRadius: BorderRadius.circular(D.rMax),
  child: BackdropFilter(
      ...
      child: Padding(
        ...
        child: Row(children: [
          Container(
            width: 48, height: 48,
            decoration: BoxDecoration(
              gradient: const LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [D.s4, D.s3]),
...
class SettingsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(
      ...
      slivers: [
        SliverToBoxAdapter(child: SizedBox(height: top + D.g8)),
        SliverToBoxAdapter(child: _FadeIn(delay: 0, child: Padding(padding: const EdgeInsets.fromLTRB(D.g20, 0, D.g20, D.g32), child: Text('Settings', style: D.hero)))),
        SliverToBoxAdapter(child: _FadeIn(delay: 50, child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: D.g16),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _SecLabel('Playback'),
            _GlassCard(children: [
              _TogRow(icon: CupertinoIcons.play_circle_fill, color: D.red,   title: 'Auto Play',   sub: 'Start streaming immediately on channel open', val: settings['autoPlay'] as bool? ?? true,   onChange: (v) => _set('autoPlay', v)),
...
class _PlayerState extends State<PlayerScreen> with TickerProviderStateMixin {
  ...
  Future<void> _initPlayer() async {
    ...
    try {
      ...
      _cc = ChewieController(
        videoPlayerController: _vpc!,
        autoPlay: widget.autoPlay, looping: true, allowFullScreen: false, allowMuting: true, showControls: false,
  ...
  void _toggleFS(bool isLand) {
    HapticFeedback.lightImpact();
    SystemChrome.setPreferredOrientations(isLand ? [DeviceOrientation.portraitUp] : [DeviceOrientation.landscapeLeft, DeviceOrientation.landscapeRight]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: AnnotatedRegion<SystemUiOverlayStyle>(
        ...
        child: isLand ? _land(mq) : _port(mq),
      ),
    );
  }
  ...
  Widget _ctrlsWidget(double topOff, bool isLand) => Stack(children: [
    Positioned(
      top: topOff + D.g10, left: 0, right: 0,
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: D.g16),
        child: Row(children: [
                  ...
                  child: BackdropFilter(
                    ...
                    child: Container(
                      ...
                      child: AnimatedSwitcher(
                        ...
                        transitionBuilder: (c, a) => ScaleTransition(scale: Tween(begin: 0.5, end: 1.0).animate(CurvedAnimation(parent: a, curve: Curves.easeOutBack)), child: FadeTransition(opacity: a, child: c)),
                        child: Icon(_isFav ? CupertinoIcons.heart_fill : CupertinoIcons.heart, key: ValueKey(_isFav), color: _isFav ? D.red : D.lbl2, size: 22),
                      ),
                    ),
                  ),
                ),
...
class _PError extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(color: Colors.black, child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    _glassCircle(const Icon(CupertinoIcons.wifi_slash, size: 34, color: Colors.white38)),
    ...
    child: BackdropFilter(
      ...
      child: Container(
        ...
        child: Stack(children: [
          Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(D.r16),
            gradient: LinearGradient(begin: Alignment.topCenter, end: const Alignment(0, 0.4), colors: [Colors.white.withValues(alpha: 0.05), Colors.transparent]),
          ))),
...
class _LivePulseState extends State<_LivePulse> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => FadeTransition(
    opacity: _a,
    child: Container(width: 6, height: 6, decoration: BoxDecoration(color: D.red, shape: BoxShape.circle, boxShadow: [BoxShadow(color: D.red.withValues(alpha: 0.65), blurRadius: 5, spreadRadius: 1)])),
  );
}
...
class _EmptyView extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    _glassCircle(Icon(icon, size: 34, color: D.lbl3)),
    const SizedBox(height: 20),
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(children: List.generate(5, (_) => AnimatedBuilder(
      ...
      builder: (_, __) => Container(
        height: 68, margin: const EdgeInsets.only(bottom: 1),
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(D.r8), gradient: LinearGradient(begin: Alignment(_x.value - 1, 0), end: Alignment(_x.value, 0), colors: [D.s2, D.s4.withValues(alpha: 0.65), D.s2])),
      ),
    ))),
  );
}
...
class _TapState extends State<_Tap> with SingleTickerProviderStateMixin {
  ...
  static const _releaseSpring = Cubic(0.34, 1.56, 0.64, 1.00);

  @override
  void initState() {
    super.initState();
    _c = AnimationController(vsync: this, lowerBound: -0.3, upperBound: 1.0);
  }
  @override void dispose() { _c.dispose(); super.dispose(); }

  void _down(_) { HapticFeedback.lightImpact(); _c.animateTo(1.0, duration: const Duration(milliseconds: 72), curve: _pressIn); }
  void _up(_)   { widget.onTap?.call(); _c.animateTo(0.0, duration: const Duration(milliseconds: 480), curve: _releaseSpring); }
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedBuilder(
      animation: _c,
      builder: (_, child) => Transform.scale(scale: 1.0 - 0.07 * _c.value, child: child),
      child: widget.child,
    ),
  );
}
...
class _FadeIn extends StatefulWidget {
  ...
  @override State<_FadeIn> createState() => _FadeInState();
}
class _FadeInState extends State<_FadeIn> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _dy  = Tween(begin: 18.0, end: 0.0).animate(CurvedAnimation(parent: _c, curve: const Cubic(0.16, 1.00, 0.30, 1.00)));
    if (widget.delay == 0) {
      _c.forward();
    } else {
        ...
        child: ClipRRect(
          ...
          child: BackdropFilter(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: D.s4.withValues(alpha: 0.88),
                borderRadius: BorderRadius.circular(D.r20),
              ...
              child: Stack(children: [
                Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(D.r20),
                  gradient: LinearGradient(begin: Alignment.topCenter, end: const Alignment(0, 0.5), colors: [Colors.white.withValues(alpha: 0.06), Colors.transparent]),
                ))),
...
PageRoute<R> _premiumRoute<R>(Widget page) => PageRouteBuilder<R>(
  ...
  transitionsBuilder: (_, a, __, child) => FadeTransition(
    ...
    child: ScaleTransition(
      scale: Tween(begin: 0.93, end: 1.0).animate(CurvedAnimation(parent: a, curve: D.snap)),
      child: child,
    ),
  ),
);
